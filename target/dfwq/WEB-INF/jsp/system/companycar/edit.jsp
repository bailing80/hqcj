...
<script type="text/javascript" src="${ctx}/js/system/companycar/edit.js"></script>
<script type="text/javascript"
	src="${ctx}/js/system/cookie/companycarEdit_cookie.js"></script>
<style type="text/css">
.col-sm-3 {
	width: 15%;
	float: left;
...
<!-- 		<li id="guaranteetab"><a href="#guarantee" data-toggle="tab">担保人</a></li>
	...
	<div id="myTabContent" class="tab-content">
		...
		<div class="tab-pane fade in active" id="car">
			...
			<form id="carform" name="form" class="form-horizontal" method="post"
				action="${ctx}/companycar/editCarEntity.shtml">
				...
				<div class="panel-body container-fluid panel--padding">
					...
					<div class="row" style="">
						<div class="form-group col-sm-6">
							...
							<div class="row">
								<input type="file" placeholder="请输入车辆照片"
					...
					<div class="" style="float: left; margin-left: 50px;">
						<textarea class="form-control checkacc" placeholder="请输入备注"
							...
							style="width: 729px; resize: none">${car.ccremark }</textarea>
					</div>
						...
						<div class="form-group col-sm-6">
							...
							<div class="row">
								<input type="text" class="form-control"
									value="${carowner.comlicence}" placeholder="请输入驾驶证号"
	...
	<script type="text/javascript">
		...
		var url = rootPath + '/companycar/selectPic.shtml?id=' + cbox;
		...
		var cbox = $.cookie("cbox");
		var url = rootPath + '/companycar/selectPicown.shtml?id=' + cbox;
		alert(cbox+"...............");
		var data = CommnUtil.ajax(url, null, "json");
		for(var i=0; i<data.length; i++){
			var showImg = "<img style=\"margin-left:14px;\" src=" + "${ctx}/uploadFile/" + data[i].comphoto +" width='180' height='140' id='10'/>";
	...
	$("#upload").click(function(){
		...
		formData.append("cplateid", $("#ccplateid").val());
		 $.ajax({
                type: "post",
    ...
    function showpic(obj) {
        ...
        reader.onload = function (e) {
            //不太明白e.target.result代表什么（一大串数字）好像是图片转化为base64
            var imgstr = '<img  style="width:180px;height:140px;margin-left:14px;" src="' + e.target.result + '"/>';
            $("div").remove(".img");
            $("img").remove("#10");
            $("#img22-follow").append(
                "<div class=\"img\">" +
...
$("#caronwersubmit").click(function(){
	...
	/* 			var carInfo = {
						carowner : {
							coname : $("#coname").val(),
							coidcard : $("#coidcard").val(),
							cosex : $("#cosex").val(),
							cobirthtime : $("#cobirthtime").val(),
							cophone : $("#cophone").val(),
							clicence : $("#clicence").val(),
							cocartype : $("#cocartype").val(),
							copapertime : $("#copapertime").val(),
							coaddress : $("#coaddress").val(),
							coserviceid : $("#coserviceid").val(),
							coservicename : $("#coservicename").val(),
							coreputationlevel : $("#coreputationlevel").val(),
							coworkcardid : $("#coworkcardid").val(),
							cocardendtime : $("#cocardendtime").val(),
							cophoto : $("#cophoto").val(),
							cremark : $("#cremark").val()
						}
				};
				...
				formData.append("cplateid", $("#ccplateid").val());
	/* 			formData2.append("jsonInfo", jsonInfo); */
				 $.ajax({
		                type: "post",
		                url: rootPath + '/companycar/picuploadedit.shtml',
		                data: formData,
		                async: false,
		                contentType: false,// jQuery不要去设置Content-Type请求头
		                processData: false,// jQuery不要去处理发送的数据
	...
	/* 	     	$.ajax({
		                type: "post",
		                url: rootPath + '/car/carownersave.shtml',
		                data: formData2,
		                async: false,
		                contentType: false,// jQuery不要去设置Content-Type请求头
		                processData: false,// jQuery不要去处理发送的数据
